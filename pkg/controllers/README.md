...
```
[PodController] -> 파드 감지 -> [Provisioner] -> 파드 배치 -> 노드 생성 -> 파드 바인딩
```
...
```
[DisruptionController] -> 후보 식별 -> 중단 결정 -> 대체 노드 생성 -> 노드 제거
```
...
```
[HealthController] -> 상태 모니터링 -> 비정상 감지 -> 복구 결정 -> 조치 실행
```
...
```
[ExpirationController] -> TTL 모니터링 -> 만료 감지 -> 노드 제거
```
